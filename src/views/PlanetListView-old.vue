...
<script lang="ts">
import { Ref, ref, onMounted, watch, computed, defineComponent } from 'vue'
      ...
      return planets.value.sort((a: IPlanet, b: IPlanet): any => {
        if (order.value === 'asc') {
          if (typeof a[orderBy.value] === 'string') {
            return String(a[orderBy.value]).localeCompare(
              String(b[orderBy.value])
            )
          } else {
          ...
          if (typeof a[orderBy.value] === 'string') {
            return String(b[orderBy.value]).localeCompare(
              String(a[orderBy.value])
            )
          } else {
